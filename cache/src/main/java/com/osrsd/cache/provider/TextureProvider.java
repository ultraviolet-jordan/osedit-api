...
public final class TextureProvider {
    ...
    public static Definition decode(ByteBuffer buffer, TextureDefinition definition) {
        buffer.getShort();
        buffer.get();
        definition.setFileIds(new int[buffer.get() & 0xff]);
        Arrays.setAll(definition.getFileIds(), id -> buffer.getShort() & 0xffff);
        return definition;
    }

    public static Definition decodeAsSprite(ByteBuffer buffer, SpriteDefinition definition) {
